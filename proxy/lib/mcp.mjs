...
async function toolSubmitReview({ card_id, mode, user_answer, rating, verdict } = {}, sub) {
  ...
  if (mode === "recite") {
    if (rating == null) {
      ...
      // The grading bar escalates with maturity: while a card is in its
      // learning steps (reps < 2 — new cards, and lapsed cards on requeue),
      // substance passes; once established, full precision is required. This
      // keeps early passes breadth-first instead of grinding one statement.
      const developing = (state.cards[card_id]?.reps ?? 0) < 2;
      return {
        needs_rating: true,
        card_id,
        rule_name: card.rule,
        canonical_statement: card.statement,
        grading_bar: developing ? "substance" : "exact",
        instructions: developing
          ? "This card is still in its learning steps, so grade for SUBSTANCE, not completeness: 3 (pass) requires only the core of the rule — the operative standard or test, the right parties, and the right outcome. Missing secondary elements, exceptions, or list items, and loose wording, must NOT pull the rating below 3 when that core is right; give 4-5 as completeness improves. Rate 0-2 only when the core is wrong: a wrong standard, wrong party, wrong outcome, or a different rule. Then call submit_review again with the same card_id, mode 'recite', and your integer rating 0-5 to record it."
          : "This card is established, so grade STRICTLY against canonical_statement: 5 = complete and precise, 4 = all elements with minor wording slips, 3 = substantively correct but one element mushy, 2 = missing an element or exception, 1 = fragments or wrong standard, 0 = wrong rule. Syntax stays lax — order, tense, and true synonyms preserving legal meaning are fine. Then call submit_review again with the same card_id, mode 'recite', and your integer rating 0-5 to record it.",
      };
    }
